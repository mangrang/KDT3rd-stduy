...
-- 4. 테이블 삭제
-- DROP: 다 삭제 (구조 자체를 삭제)
-- TRAUNCATE: 초기화 (모든 행 삭제)
DROP TABLE user;
TRUNCATE TABLE user;

-- SQL문은 snake_case문 사용
-- 5. 테이블 수정 (구조 수정)
-- 5-1. 컬럼 추가 (ADD)
...
-- [Create - INSERT]
-- : 데이터 추가
-- 참고) AUTO_INCREAMENT 칼럼의 값은 굳이 넣지 않으면 알아서 증가
-- NULL 이라고 넣으면 알아서 숫자가 지정되기도 함
-- 주의) AUTO_INCREAMENT 컬럼은 반드시 PK(기본키, primary key)여야 함
...
INSERT INTO user (name, age, address) VALUES ('권희수', 36, '서울특별시 마포구');
...
SELECT id, name FROM user WHERE name = '이지은'; -- 이름이 '이지은'인 사람의 id, age 조회


-- ORDER BY 절: 데이터 정렬
-- ASC: 오름차순 (기본값)
-- DESC: 내림차순
SELECT * FROM user ORDER BY age DESC; -- age 오름차순
SELECT * FROM user WHERE id > 6 ORDER BY age ASC; -- id 6 이상인 사람 중에서 age 컬럼 오름차순
SELECT * FROM user ORDER BY name ASC; -- name 컬럼 오름차순 
...
-- 구문에서 이름이 4글자이거나 2글자인 사람은 제외되는 건가?
SELECT * FROM user WHERE name LIKE '%희%'; -- 이름에 '희'가 포함된 데이터
SELECT * FROM user WHERE address LIKE '%광역%'; -- 주소에 '광역'이 포함된 데이터
SELECT * FROM user WHERE address LIKE '__희' ORDER BY age DESC;
...
SELECT * FROM user WHERE name LIKE '이%' and age = 23; -- 이씨인 23세인 사람


-- [Update - UPDATE]
-- : 데이터 갱신(수정)
-- 참고) Error Code: 1175.
-- 테이블 데이터를 update/delete 하려고 할 때 key 컬럼을 이용해서 수정/삭제하는 것이 바람직
UPDATE user SET address = '서울특별시 강북구' WHERE id = 6;
UPDATE user SET address = '제추틀별자치도 제주시', name = '이지헌' WHERE id = 2;
